...
const HotelEditForm = (props) => {
    ...
    useEffect(() => {
        ...
        var control = new TomSelect('#loca', conf)
    ...
    return (<>
        <form onSubmit={handleSubmit}>
            <div className='form-group'>
                ...
                <select id='loca' name='location' onChange={handleChange} type='text' className='form-control m-2' placeholder='Location' >
                    <option value={location} >{location}</option>
                </select>
                <input type='number' onChange={handleChange} className='form-control m-2' value={price} name='price' placeholder='Price' ></input>

                <Select defaultValue={bed} placeholder='Number of Beds' className='form-control m-2' onChange={(e) => setValues({ ...values, bed: e })} >
                ...
                <DatePicker defaultValue={dayjs(from, 'YYYY-MM-DD')} placeholder='from date' disabledDate={(current) => current && current.valueOf() < moment().subtract(1, 'days')} className='form-control m-2' onChange={(date, dateString) => setValues({ ...values, from: dateString })} />
                <DatePicker defaultValue={dayjs(from, 'YYYY-MM-DD')} placeholder='to date' disabledDate={(current) => current && current.valueOf() < moment().subtract(1, 'days')} className='form-control m-2' onChange={(date, dateString) => setValues({ ...values, to: dateString })} />
